-- ============================================================
-- Migration: skill tags + first blood on challenges/submissions
-- Run in Supabase SQL Editor.
-- ============================================================

-- Skill tags on challenges (array of strings, e.g. ["log-analysis","smtp","mongodb"])
ALTER TABLE public.challenges
  ADD COLUMN IF NOT EXISTS tags jsonb NOT NULL DEFAULT '[]';

-- First blood flag on submissions
ALTER TABLE public.submissions
  ADD COLUMN IF NOT EXISTS first_blood boolean NOT NULL DEFAULT false;

-- Update submit_challenge RPC to detect first blood
...
begin
  ...
  -- First blood: no other user has solved this yet
  select not exists (
    select 1 from public.submissions where challenge_id = p_challenge_id
  ) into v_is_first_blood;
  ...
  insert into public.submissions (user_id, challenge_id, score_earned, hints_cost, answers, first_blood)
  values (v_uid, p_challenge_id, v_total_earned, v_hints_cost, v_graded, v_is_first_blood);
